...
// Title (0)
// Filename (14)
// AudioLength (8) // In seconds usually from what I saw
// Keywords (18)
// Mood (19)
...
for (let i = 1; i < rows.length; i++) {
    ...
    // In case of parsing issues with ';' inside strings, doing a basic check
    ...
    if (filename && targetFiles.has(filename.trim())) {
        ...
        let imageName = filename.trim().replace('.mp3', '.webp');
        ...
        results.push({
            ...
            keywords: cols[keywordsIdx] ? cols[keywordsIdx].trim().split(',').map(s=>s.trim()).filter(Boolean) : [],
            mood: cols[moodIdx] ? cols[moodIdx].trim().split(',').map(s=>s.trim()).filter(Boolean) : []
        });
    }
}

// Find if any were missed
const foundFiles = new Set(results.map(r => r.filename));
const missedFiles = userFilesList.filter(f => !foundFiles.has(f.trim()));
if (missedFiles.length > 0) {
    console.log("Missed files:", missedFiles);
}

fs.writeFileSync('d:/0CODE/AIstudio/dnd5-azazel/src/data/musics.json', JSON.stringify(results, null, 2));
console.log(`Generated musics.json with ${results.length} items`);
